...
const CACHE_DIR = require('./config').CACHE_DIR;
...
`, { timeout: 20000 });
  ...
  async minimizeApp(payload) {
    ...
    if (!id) return { ok: false, code: 1, stdout: '', stderr: 'ID programma mancante', timedOut: false };
    return runPS(`
Add-Type @"
using System;
using System.Runtime.InteropServices;
...
if (-not $proc) { Write-Output "Programma non trovato"; exit 0 }
[Win]::ShowWindowAsync($proc.MainWindowHandle, 6) | Out-Null
Write-Output "Ridotto a icona (${id})"
`, { timeout: 15000 });
  },
...
$proc = Get-Process -Id ${id} -ErrorAction SilentlyContinue
if (-not $proc) { Write-Output "Programma non trovato"; exit 0 }
[Win]::ShowWindowAsync($proc.MainWindowHandle, 9) | Out-Null
[Win]::ShowWindowAsync($proc.MainWindowHandle, 3) | Out-Null
[Win]::SetForegroundWindow($proc.MainWindowHandle) | Out-Null
Write-Output "A schermo intero (${id})"
`, { timeout: 15000 });
  },
